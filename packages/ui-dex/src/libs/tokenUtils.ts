...
export function tokenVolumesInUsd(
  token: Token,
  timeframe: string,
): number | string | undefined {
  switch (timeframe) {
...
type TokenSort =
  | Pick<GetHotTokensRequest, "sortBy" | "sortDirection">
  | Pick<GetNewTokensRequest, "sortBy" | "sortDirection">
  | Pick<GetStocksTokensRequest, "sortBy" | "sortDirection">;
...
export function tokenFilters(
  ...
  Object.entries(filters).forEach(([key, value]) => {
    const field = tokenFieldsByKey(key, timeframe) as FilterConditionFieldEnum;
    if (!field) return;
    if (value.includes(":")) {
