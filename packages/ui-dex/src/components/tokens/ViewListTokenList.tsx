...
export function ViewListTokenList() {
  return (
    <>
      <ViewListTokenListHeaders />
      <AuthGuard>
        <ViewListTokenListContent />
      </AuthGuard>
    </>
  );
}

function ViewListTokenListContent() {
  ...
  if (tokens === undefined || isLoading || (isFetching && resetTokensRef.current)) {
    return <ViewListTokenListSkeleton />;
  }
...
function ListItem({ token, isViewed, onView, onUnview }: ListItemProps) {
  const layout = useAtomValue(layoutAtom);

  const { navigate } = useRouter();

  const onToggleView = useAuthenticatedCallback(() => {
    if (isViewed) {
      onUnview(token.address);
    } else {
      onView(token.address);
    }
  }, [isViewed, onView, onUnview, token.address]);
  ...
  return (
    ...
    >
      {layout === "desktop" && (
        <>
          <ViewListField isViewed={isViewed} onAction={onToggleView} token={token} />
      ...
      {layout !== "desktop" && (
        <>
          ...
          <ViewListMobileField isViewed={isViewed} onAction={onToggleView} token={token} />
        </>
      )}
    </div>
  );
