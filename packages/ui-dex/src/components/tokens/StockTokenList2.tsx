...
export function StockTokenList2({ height }: { height?: number }) {
  return (
    <>
      {/* <StockTokenListHeaders /> */}
      <StockTokenListContent height={height} />
    </>
  );
}

function StockTokenListContent({ height }: { height?: number }) {
  ...
  useEffect(() => {
    ...
    const subscribeTokenStats = dexClient.stream.subscribeRankingTokensStats({
      ...
      channelType: ChannelType.US_STOCKS,
