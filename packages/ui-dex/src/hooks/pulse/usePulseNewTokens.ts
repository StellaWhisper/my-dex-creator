...
export function usePulseNewTokens() {
  ...
  const handleStats = useCallback(
    (_stats: TokenStat[]) => {
      // const tokens = stats.map((it) => convertStreamRankingTokenStat(it));
      // mergeTokens(tokens);
    },
    [],
  );
  ...
  const handleHoldings = useCallback(
    (_holders: TokenHolder[]) => {
      // const tokens = holders.map((it) => convertStreamRankingTokenHolders(it));
      // mergeTokens(tokens);
    },
    [],
  );
  ...
  const handleSupply = useCallback(
    (_supplies: TokenSupply[]) => {
      // const tokens = supplies.map((supply) =>
      //   convertStreamRankingTokenSupply(supply, primaryTokenPriceRef.current ?? 0),
      // );
      // mergeTokens(tokens);
    },
    [],
  ...
  const handleLiquidity = useCallback(
    (_liquidities: TokenLiquidity[]) => {
      // const tokens = liquidities.map((it) => convertStreamRankingTokenLiquidity(it));
      // mergeTokens(tokens);
    },
    [],
  );
