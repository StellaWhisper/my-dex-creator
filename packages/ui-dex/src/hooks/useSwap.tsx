...
import {
  ResponseError,
  SendTxInputSubmitTypeEnum,
  SwapRouteInputDexEnum,
  SwapRouteInputSwapModeEnum,
...
export function useSwap() {
  ...
  const swap = useCallback(
    async ({
      ...
      if (!user?.solanaAddress) return false;
      ...
      try {
        ...
        const { serializedTx } = await fetchSwapRoute(dexClient, {
          chain,
          swapRouteInput: {
            dex: SwapRouteInputDexEnum.Jupiter,
            // TODO using wallet interface instead
            userAddress: user.solanaAddress,
            amount: new BigNumber(amount).toString(),
            swapMode: SwapRouteInputSwapModeEnum.ExactIn,
            slippage,
            priorityFee,
            tipFee,
            isAntiMev:
        ...
        // sign tx
        const signedTxBuffer = await signTransaction(Buffer.from(serializedTx, "base64"));
        ...
        const { signature, jobId } = await sendTransaction(dexClient, {
          ...
          sendTxInput: {
            signedTx,
            submitType: SendTxInputSubmitTypeEnum.Default,
            options: { isAntiMev },
          },
        });
    ...
    [dexClient, chain, user, signTransaction, waitSwapConfirmation, toast, t],
  );
