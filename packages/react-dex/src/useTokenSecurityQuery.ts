...
export async function fetchTokenSecurity(client: DexClient, chain: CHAIN_ID, tokenAddress: string) {
  return await client.token.getSecurity({ chain: chainParam(chain), tokenAddress });
}
