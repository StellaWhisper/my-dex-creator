...
import {
  TokenStat as StreamTokenStat,
  TokenHolder as StreamTokenHolder,
  TokenSupply as StreamTokenSupply,
  TokenLiquidity as StreamTokenLiquidity,
  RankingTokenList,
  NewToken,
  TokenMetadata,
} from "@chainstream-io/sdk/stream";
import { Token, TokenMarketData, TokenStat } from "@chainstream-io/sdk/openapi";
...
export function convertStreamTokenStat(stat: StreamTokenStat): Partial<TokenStat> {
  return {
    ...
    volumesInUsd24h: new BigNumber(stat.buyVolumeInUsd24h ?? 0)
      ...
      .toString(),
    trades1m: new BigNumber(stat.buys1m ?? 0).plus(stat.sells1m ?? 0).toNumber(),
    trades5m: new BigNumber(stat.buys5m ?? 0).plus(stat.sells5m ?? 0).toNumber(),
    trades1h: new BigNumber(stat.buys1h ?? 0).plus(stat.sells1h ?? 0).toNumber(),
    trades4h: new BigNumber(stat.buys4h ?? 0).plus(stat.sells4h ?? 0).toNumber(),
    trades24h: new BigNumber(stat.buys24h ?? 0).plus(stat.sells24h ?? 0).toNumber(),
    traders1m: new BigNumber(stat.buyers1m ?? 0).plus(stat.sellers1m ?? 0).toNumber(),
    traders5m: new BigNumber(stat.buyers5m ?? 0).plus(stat.sellers5m ?? 0).toNumber(),
    traders1h: new BigNumber(stat.buyers1h ?? 0).plus(stat.sellers1h ?? 0).toNumber(),
    traders4h: new BigNumber(stat.buyers4h ?? 0).plus(stat.sellers4h ?? 0).toNumber(),
    traders24h: new BigNumber(stat.buyers24h ?? 0).plus(stat.sellers24h ?? 0).toNumber(),
...
export function convertStreamTokenHoldersToMarketData(
  ...
  return {
    ...
    top10HoldingsRatio: holder.top10Ratio,
    top100HoldingsRatio: holder.top100Ratio,
  };
}
...
export function convertStreamRankingTokenHolders(
  ...
  return {
    ...
    marketData: {
      ...
      top10HoldingsRatio: holders.top10Ratio,
      top100HoldingsRatio: holders.top100Ratio,
    },
  };
}
...
export function convertStreamRankingToken(
  ...
  return {
    ...
    marketData: {
      ...
      // holdings etc.
      holders: rankingToken.holder?.holders,
      top10TotalHoldings: rankingToken.holder?.top10Amount,
      top10HoldingsRatio: rankingToken.holder?.top10Ratio,
      top100TotalHoldings: rankingToken.holder?.top100Amount,
      top100HoldingsRatio: rankingToken.holder?.top100Ratio,
