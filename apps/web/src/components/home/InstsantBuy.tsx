...
export function InstantBuy({ token }: TokenListActionsProps) {
  ...
  const primaryTokenSymbol = useMemo(() => getPrimaryTokenSymbol(CHAIN_ID.SOLANA), []);

  const primaryTokenDecimals = useMemo(() => getPrimaryTokenDecimals(CHAIN_ID.SOLANA), []);

  const primaryTokenAddress = useMemo(() => getPrimaryTokenAddress(CHAIN_ID.SOLANA), []);

  const buySettings = useTradeBuySettings(CHAIN_ID.SOLANA);
